...
2. **CNAME ファイルの確認**:
   - リポジトリのルートに `CNAME` ファイルが自動生成され、`aoma23.com` が記載されていることを確認します。
...
GitHub Pages と Squarespace を組み合わせることで、無料で独自ドメインの静的サイトを運用できます。  
設定は少し手間がかかりますが、一度行えば安定して運用できます。  
今後は `aoma23.com` でブログを更新していきますので、引き続きよろしくお願いします。

---

この記事が、同様の移行を検討している方の参考になれば幸いです。
